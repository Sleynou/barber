...
router.get('/getServiceParId', async (req, res) => {
  try {
    const { idService } = req.query; // Corregido para obtener idService de req.body
    const services = await db('Services').select('*').where({ idService: idService }); // Corregido para usar la condición adecuada
    res.send(services);
  } catch (error) {
