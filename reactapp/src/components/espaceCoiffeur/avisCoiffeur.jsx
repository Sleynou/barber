...
const AvisCoiffeur = ({ idCoiffeur }) => {
    ...
    useEffect(() => {
        const fetchAvis = async () => {
            try {
                const response = await axios.get('http://localhost:3000/voirAvisParidCoiffeur', {
                    params: { idCoiffeur: 1 }
                });
...
console.log(currentAvis)
    return (
        ...
        <Container style={{ marginTop: '12em', marginBottom: '6em'}}>
        <Header as="h2" style={{ fontSize: '3.5em', marginBottom: '1em', textAlign: 'center'}}>AVIS DU: {currentAvis[0].nomSalon} </Header>
