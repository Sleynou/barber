...
function ConnexionSalon() {
  ...
  const connexion = async () => {
    try {
      const response = await fetch('http://localhost:3000/loginSalon', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        sessionStorage.setItem('idSalon', data.salonID);
        navigate(`/horraireSalon/${data.salonID}`);
      } else {
        alert('Echec de connexion !');
        console.log("erreur il faut reesayer")
      ...
      <div className="login-container" style={{ display: 'flex',width: '48.5%' , padding: '2em', borderRadius: '10px', backgroundColor: 'rgb(242 245 249)', flexGrow: 1 }}>
        <div className="login-form" style={{ flex: 1 }}>
          ...
          <Form onSubmit={connexion}>
            ...
            <Header as="h2" style={{ fontSize: '3.5em', marginBottom: '1em', marginLeft: "28.5%"}}>Connexion Salon</Header>
            ...
            <Form.Field style={{marginLeft: '43%'}}>
                ...
                <p >Pas encore inscrit ? <NavLink to="/inscriptionSalon">Inscription</NavLink></p>
            </Form.Field>
          </Form>
        </div>
      </div>
